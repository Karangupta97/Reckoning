...
function mapGovStatus(status: string): "Passed" | "Pending" | "Failed" {
...
export default function ComplianceReviewPage() {
  ...
  const govRows = governance.map((g) => ({
    id: g.id,
    entity: g.district,
    type: "Governance Request",
    status: mapGovStatus(g.status),
    dueDate: g.submittedOn,
    reviewer: "Compliance Dept",
    href: `/super-admin/governance/district-requests/${g.id}`,
  }));

  const reviews = [
    ...govRows,
    ...CONTRACTOR_REVIEWS.map((r) => ({ ...r, href: null as string | null })),
  ];
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text-secondary)]">
              District governance requests and periodic contractor compliance audits
            </p>
          </div>
        </div>
        <Link href="/super-admin/governance/district-requests" className="text-xs text-cyan-400 hover:underline shrink-0">
          District Requests →
        </Link>
      </motion.div>
        ...
        <DashboardCard>
          <div className="dashboard-table-scroll">
            <table className="dashboard-table">
              <thead>
                <tr>
                  {["Review ID", "Entity", "Type", "Status", "Due Date", "Reviewer"].map((h) => (
                  ...
                  return (
                    <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }} className="dashboard-table-row">
                      <td className="dashboard-table-td dashboard-table-td-mono text-xs">
                        {r.href ? (
                          <Link href={r.href} className="text-cyan-400 hover:underline">
                            {r.id}
                          </Link>
                        ) : (
                          r.id
