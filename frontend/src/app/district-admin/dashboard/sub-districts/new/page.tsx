...
/** Real Mumbai sub-districts for the dropdown */
...
/** Combined sub-district list — all available options */
const ALL_SUB_DISTRICT_OPTIONS = [
  ...MUMBAI_SUB_DISTRICT_NAMES,
  ...RAIGAD_SUB_DISTRICT_NAMES,
];
...
/**
 * Fetch a sub-district boundary from Nominatim (OpenStreetMap) as a GeoJSON Polygon.
 * Falls back to a bounding-box polygon if no detailed boundary is available.
 */
async function fetchBoundaryFromNominatim(
  subDistrictName: string,
): Promise<{ type: "Polygon"; coordinates: number[][][] } | null> {
  try {
    const query = `${subDistrictName}, Mumbai, Maharashtra, India`;
    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&polygon_geojson=1&limit=1`;

    const res = await fetch(url, {
      headers: { "User-Agent": "Reckoning-Admin/1.0" },
    });
    ...
    // If we got a polygon or multipolygon back from Nominatim
    if (result.geojson) {
      ...
      if (result.geojson.type === "MultiPolygon") {
        ...
        const largest = coords.reduce((max, ring) =>
          ring[0].length > max[0].length ? ring : max
        );
        return { type: "Polygon", coordinates: largest };
      }
    }

    // Fallback: build a polygon from the bounding box
...
export default function NewSubDistrictAdminPage() {
  ...
  const [createdUser, setCreatedUser] = useState<CreatedResult | null>(null);
  ...
  // Fetch existing sub-districts from backend
  ...
  // Auto-fetch boundary when sub-district is selected
  const handleSubDistrictChange = useCallback(async (name: string) => {
    ...
    if (boundary) {
      setFetchedBoundary(boundary);
      setBoundaryStatus("success");
    } else {
      setBoundaryStatus("error");
    }
  }, []);

  // Real invite mutation
  const inviteMutation = useMutation({
    mutationFn: async (data: {
      email: string;
      subDistrict: string;
      geofence: { type: "Polygon"; coordinates: number[][][] };
    }) => {
    ...
    onSuccess: (data) => {
      setCreatedUser({
        id: data.adminId || data.subDistrictId || "N/A",
        email: form.email,
    ...
    onError: (err: any) => {
      setErrors({
        email: err.response?.data?.error?.message || "Failed to invite sub-district admin",
      });
    },
  });
  ...
  const validate = (): FormErrors => {
    ...
    if (!fetchedBoundary && boundaryStatus !== "fetching") {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    inviteMutation.mutate({
      email: form.email,
      subDistrict: form.subDistrict,
      geofence: fetchedBoundary,
    });
  };

  const handleCopy = () => {
    if (!createdUser) return;
    const text = `Invitation ID: ${createdUser.id}\nEmail: ${createdUser.email}\nRole: ${createdUser.role}\nSub-District: ${createdUser.subDistrict}\nActivation: Sent via email`;
      ...
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-5">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-center max-w-sm">
          ...
          <p className="mt-2 text-sm text-[var(--color-text-secondary)]">
            Account pending onboarding. Activation email sent.
          </p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          {[
            { label: "Invitation ID", value: createdUser.id },
            ...
            { label: "Boundary", value: "Geo-fetched ✓" },
          ].map((r) => (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="flex gap-3">
          <button onClick={handleCopy}
            ...
            {copied ? <><Check size={12} /> Copied!</> : <><Copy size={12} /> Copy Details</>}
          </button>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--color-text-secondary)]">
            Onboard new officer for {districtName} District • Boundary auto-fetched from OpenStreetMap
          </p>
        </div>
      </motion.div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="rounded-lg border px-3 py-2.5 flex items-start gap-2"
              ...
              <p className="text-[11px] text-[var(--color-text-secondary)] leading-relaxed">
                System generates a User ID and temporary password. The new admin must change password on first login before accessing the dashboard. Boundary polygon is geo-fetched from OpenStreetMap Nominatim.
              </p>
            </div>
            ...
            {/* Sub-District — with real Mumbai options */}
            <div>
              <label className="text-xs font-semibold text-[var(--color-text-secondary)] flex items-center gap-1.5 mb-1.5">
            ...
            {form.subDistrict && (
              <div className="rounded-lg border px-3 py-2.5 flex items-center gap-2"
                style={{
                  borderColor: boundaryStatus === "success" ? "rgba(20,184,166,0.3)"
                    : boundaryStatus === "error" ? "rgba(239,68,68,0.3)"
                    : "rgba(59,130,246,0.2)",
                  background: boundaryStatus === "success" ? "rgba(20,184,166,0.04)"
                    : boundaryStatus === "error" ? "rgba(239,68,68,0.04)"
                    : "rgba(59,130,246,0.04)",
                }}>
                {boundaryStatus === "fetching" && (
                  <>
                    <Loader2 size={13} className="text-blue-400 animate-spin" />
                    <span className="text-[11px] text-blue-400">Fetching boundary from OpenStreetMap…</span>
                  </>
                )}
                {boundaryStatus === "success" && fetchedBoundary && (
                  <>
                    <Globe size={13} className="text-teal-400" />
                    <span className="text-[11px] text-teal-400">
                      Boundary fetched — {fetchedBoundary.coordinates[0].length} vertices
                    </span>
                  </>
                )}
                {boundaryStatus === "error" && (
                  <>
                    ...
                    <span className="text-[11px] text-red-400">
                      Could not fetch boundary. Try a different sub-district name.
                    </span>
                  </>
                )}
              </div>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[var(--color-text-muted)] w-20 shrink-0">Boundary</span>
                <span className="text-[10px] text-[var(--color-text-secondary)]">
                  {boundaryStatus === "success" ? "Geo-fetched from OSM ✓" : "Pending selection"}
                </span>
              </div>
              <div className="flex items-center gap-2">
