...
import {
  ...
  Landmark, Award, Gift, Clock, BarChart3, ChevronRight,
} from "lucide-react";
import {
...
function BadgeCard({ badge, index }: { badge: AdminBadge; index: number }) {
  ...
  const IconMap: Record<string, React.ReactNode> = {
    ...
    award: <Award size={18} />, landmark: <Landmark size={18} />, shield: <ShieldCheck size={18} />,
    zap: <Zap size={18} />,
  };

  return (
    <motion.div custom={index} variants={fadeUp} initial="hidden" animate="visible"
      className="relative flex flex-col items-center text-center rounded-xl px-2 py-3 transition-all duration-200 hover:scale-[1.03]"
      style={{
...
const SCORING_RULES = [
  ...
  { action: "Escalation Closed",           xp: 80,    color: "#F59E0B" },
  { action: "Evidence Verified",           xp: 30,    color: "#3B82F6" },
  { action: "SLA Achieved",                xp: 40,    color: "#14b8a6" },
  { action: "Governance Request Processed",xp: 60,    color: "#8B5CF6" },
  { action: "Budget Approval Completed",   xp: 70,    color: "#F97316" },
  ...
  { action: "Complaint Rejected (penalty)",xp: -10,   color: "#EF4444" },
];

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function DistrictAchievementsPage() {
  const { district } = useAchievementStore();
  const { districtOfficers } = useLeaderboardStore();
  const isClient = useIsClient();

  const [chartPeriod, setChartPeriod] = useState<"month" | "year">("month");

  const profile    = district;
  const ranks      = DISTRICT_RANKS;
    ...
    <div className="flex flex-col gap-5 pb-8">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <DashboardCard className="p-6 relative overflow-hidden"
          style={{ background: `linear-gradient(135deg, ${rank.color}0d 0%, var(--color-card) 60%)`, borderColor: `${rank.color}25` }}>
          <div className="absolute top-0 right-0 w-48 h-48 rounded-full pointer-events-none opacity-[0.06]"
            style={{ background: rank.color, filter: "blur(40px)" }} />
          <div className="relative flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-end justify-between gap-3 mb-2.5 flex-wrap">
                <div>
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-sm font-semibold text-teal-400">XP</span>
                  </div>
              ...
              <div className="relative w-full h-3 rounded-full overflow-hidden border border-[var(--color-border)]"
                ...
                <motion.div initial={{ width: 0 }} animate={{ width: `${xpPct}%` }}
                  ...
                  style={{ background: `linear-gradient(90deg, ${rank.color}, #14b8a6)`, boxShadow: `0 0 10px ${rank.color}50` }}>
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent" />
                </motion.div>
              </div>
              <div className="flex justify-between mt-1">
                <span className="text-[10px] text-[var(--color-text-muted)]">{bandStart.toLocaleString()} XP</span>
                <span className="text-[10px] font-medium" style={{ color: rank.color }}>{xpPct}% complete</span>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
        ...
        {[
          ...
          { id: "streak", label: "Contribution Streak",value: `${profile.streak} Days`,               color: "#F97316", sub: "Keep it up!", subColor: "#F97316", icon: <Flame size={18}/> },
          { id: "rank",   label: "District Rank",      value: `#${profile.rankPosition}`,             color: "#3B82F6", sub: "District-wide", subColor: "#3B82F6", icon: <BarChart3 size={18}/> },
        ].map((s, i) => (
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}>
        <DashboardCard className="p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">Rank Progress</h3>
          </div>
          <RankProgressTrack ranks={ranks} currentRank={rank} accentColor={rank.color} />
        </DashboardCard>
      </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}>
            <DashboardCard className="p-5">
              ...
              <div className="flex items-center gap-4 mb-4">
                <LegendDot color="#14b8a6" label="Resolved" />
                <LegendDot color="#F59E0B" label="Escalations" />
                ...
                {isClient && (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={profile.monthlyContribution} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                      <defs>
                        {[["teal","#14b8a6"],["amber","#F59E0B"],["blue","#3B82F6"]].map(([id, c]) => (
                          <linearGradient key={id} id={`grad-da-${id}`} x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Area type="monotone" dataKey="resolved"    stroke="#14b8a6" strokeWidth={2} fill="url(#grad-da-teal)"  dot={false} activeDot={{ r: 4 }} />
                      <Area type="monotone" dataKey="escalations" stroke="#F59E0B" strokeWidth={2} fill="url(#grad-da-amber)" dot={false} activeDot={{ r: 4 }} />
                      <Area type="monotone" dataKey="evidence"    stroke="#3B82F6" strokeWidth={2} fill="url(#grad-da-blue)"  dot={false} activeDot={{ r: 4 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <DashboardCard className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="text-xs text-teal-400 hover:underline">See details</button>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {profile.impactStats.map((stat, i) => {
                  const COLORS = ["#3B82F6","#22C55E","#F59E0B","#8B5CF6","#06B6D4","#F97316"];
                  const c = COLORS[i % COLORS.length];
                  const IconMap: Record<string, React.ReactNode> = {
                    "check-circle": <CheckCircle2 size={17} />, shield: <ShieldCheck size={17} />,
                    camera: <Camera size={17} />, "shield-check": <ShieldCheck size={17} />,
                    trophy: <Trophy size={17} />, landmark: <Landmark size={17} />,
                  };
                  return (
                    <motion.div key={stat.label} custom={i} variants={fadeUp} initial="hidden" animate="visible"
                      className="flex items-center gap-2.5 p-3 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-teal-400/30 transition-colors">
                ...
                {SCORING_RULES.map(r => (
                  <div key={r.action} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
                    ...
                    <span className="text-xs font-bold tabular-nums font-mono"
                      style={{ color: r.xp < 0 ? "#EF4444" : r.color }}>
                      {r.xp > 0 ? "+" : ""}{r.xp} XP
                    </span>
                  </div>
                ))}
        ...
        <div className="flex flex-col gap-5">
            ...
            <DashboardCard className="p-5">
              ...
              <div className="flex flex-col gap-3">
                {profile.challenges.map(ch => {
                  ...
                  return (
                    <div key={ch.id} className="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:border-teal-400/30 transition-colors">
                      ...
                      <div className="relative w-full h-1.5 rounded-full overflow-hidden bg-[var(--color-border)]">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }}
                          ...
                          style={{ background: done ? "#22C55E" : "linear-gradient(90deg, #14b8a6, #06b6d4)" }} />
                      </div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}>
            <DashboardCard className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="text-xs text-teal-400 hover:underline">View all</button>
              </div>
              <AchievementTimeline events={profile.timeline} accentColor="#14b8a6" />
            </DashboardCard>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            <DashboardCard className="p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">Leaderboard Preview</h3>
                <a href="/district-admin/leaderboard"
                ...
                {top3.map((e, i) => (
                  <div key={e.rank} className="flex items-center gap-2.5 p-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[var(--color-text-muted)]">{e.casesResolved} resolved</p>
                    </div>
                    <span className="text-xs font-black font-mono text-teal-400">{e.points.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </DashboardCard>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }}>
            <div className="relative overflow-hidden rounded-2xl p-5"
              style={{ background: "linear-gradient(135deg, #0d9488 0%, #14b8a6 50%, #22C55E 100%)", boxShadow: "0 8px 24px rgba(20,184,166,0.25)" }}>
              ...
              <div className="relative">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-base font-bold text-white">Exceptional Work!</h3>
                </div>
                <p className="text-sm text-white/90">
                  Your district has resolved{" "}
                  <span className="font-bold text-white">{profile.impactStats[0].value} complaints</span>{" "}
