...
export async function verifyOtp(
  ...
  try {
    const result = await authService.verifyOtp(req.body as VerifyOtpBody);
...
export async function resendOtp(
  ...
  try {
    const result = await authService.resendOtp(req.body as ResendOtpBody);
    res.status(200).json(result);
  } catch (error) {
    next(error);
  }
...
export async function logout(
  ...
  try {
    ...
    const { refreshToken, allDevices } = req.body as LogoutBody;
