...
import type { Country, IssueCategory, SeverityLevel } from "@prisma/client";
...
const CRITICAL_CATEGORIES: ReadonlySet<IssueCategory> = new Set<IssueCategory>([
  "POTHOLE",
  "CRACKS_DAMAGE",
]);
...
/** Inputs to the severity heuristic. */
 ...
 */
export function computeSeverity(inputs: SeverityInputs): SeverityLevel {
  const { category, aiConfidence, description, suggestedFix } = inputs;

  let base: SeverityLevel;
 ...
 */
const COUNTRY_BOUNDS: Readonly<
  Record<Country, { minLat: number; maxLat: number; minLng: number; maxLng: number }>
> = {
...
/** Result of a coordinate sanity check. */
...
export function validateCoordinates(
  ...
  if (
    latitude < BIMSTEC_BOUNDS.minLat ||
