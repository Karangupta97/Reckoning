 ...
 */
async function buildRedisStore(): Promise<Store | undefined> {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[rateLimiter] Redis store unavailable; falling back to in-memory store.",
...
export const registerLimiter: RateLimitRequestHandler = makeLimiter(15 * MINUTES, 5);

/** `POST /api/auth/verify-otp` → 10 requests / 15 minutes / IP. */
export const verifyOtpLimiter: RateLimitRequestHandler = makeLimiter(15 * MINUTES, 10);
...
function ipEmailKey(req: Request): string {
  ...
  const email =
    body && typeof body.email === "string" ? body.email.toLowerCase() : "unknown";
  return `${ip}:${email}`;
}
 ...
 */
export const loginIpLimiter: RateLimitRequestHandler = makeLimiter(15 * MINUTES, 10);
...
export const refreshLimiter: RateLimitRequestHandler = makeLimiter(15 * MINUTES, 20);
