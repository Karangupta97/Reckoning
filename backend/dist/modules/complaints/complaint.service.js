 ...
 */
function toMediaViews(complaint) {
 ...
 */
function reporterLabel(isAnonymous, fullName) {
 ...
 */
async function nextTicketSequence(tx, country, year) {
 ...
 */
async function validateMediaOwnership(mediaIds, userId) {
    ...
    const rows = await dbGuard(() => prisma.mediaUpload.findMany({
        where: { id: { in: unique } },
 ...
 */
async function findAssignedAuthority(country, latitude, longitude) {
    const rows = await dbGuard(() => prisma.$queryRaw `
        SELECT id
        FROM "authorities"
        WHERE "isActive" = true
 ...
 */
async function detectDuplicate(userId, category, latitude, longitude) {
    const rows = await dbGuard(() => prisma.$queryRaw `
        SELECT "ticketNumber"
        FROM "complaints"
        WHERE "userId" = ${userId}
 ...
 */
export async function createComplaint(userId, input, _ipAddress) {
    ...
    if (!isWithinCountryBounds(country, latitude, longitude)) {
        throw new AppError("Report location is outside your registered country.", 400, { code: "COUNTRY_MISMATCH" });
    }
    ...
    if (geo?.countryCode && geo.countryCode !== isoCodeForCountry(country)) {
        throw new AppError("Report location does not match your registered country.", 400, { code: "COUNTRY_MISMATCH" });
    }
    ...
    const aiDetected = input.aiCategory !== undefined || input.aiConfidence !== undefined;
    const year = new Date().getUTCFullYear();
    // 5 + 6. Atomic create: ticket sequence → complaint → location → media links.
    const complaint = await dbGuard(() => prisma.$transaction(async (tx) => {
 ...
 */
async function nearbyComplaintIds(q, offset) {
    const params = [q.lng, q.lat, q.radius];
    const conditions = [
        `"deletedAt" IS NULL`,
    ...
    if (q.category)
        addFilter(`category`, q.category, "::text");
    if (q.status)
        addFilter(`status`, q.status, "::text");
    if (q.severity)
        addFilter(`severity`, q.severity, "::text");
    if (q.country)
        addFilter(`country`, q.country, "::text");
    if (q.startDate) {
    ...
    const countResult = await dbGuard(() => query(`SELECT COUNT(*)::text AS count FROM "complaints" WHERE ${where}`, params), "nearbyComplaintIds:count");
    ...
    const rowsResult = await dbGuard(() => query(`SELECT id,
                ST_Distance(location, ST_SetSRID(ST_MakePoint($1, $2), 4326)::geography) AS distance
         FROM "complaints"
         WHERE ${where}
         ${orderBy}
         LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, pageParams), "nearbyComplaintIds:rows");
 ...
 */
function toListItem(c, fullName, distance) {
...
export async function listComplaints(q) {
    ...
    if (q.lat !== undefined && q.lng !== undefined) {
        ...
        const records = ids.length
            ? await dbGuard(() => prisma.complaint.findMany({
                where: { id: { in: ids } },
    ...
    const [total, records] = await dbGuard(() => prisma.$transaction([
...
function toDetail(c, requester) {
    ...
    if (isOwner)
        detail.isAnonymous = c.isAnonymous;
    return detail;
}
/**
 ...
 */
export async function getComplaintById(id, requester) {
    const complaint = await dbGuard(() => prisma.complaint.findUnique({
        where: { id },
        include: { ...COMPLAINT_INCLUDE, user: { select: { fullName: true } } },
    ...
    await dbGuard(() => prisma.complaint.update({ where: { id }, data: { viewCount: { increment: 1 } }, select: { id: true } }), "getComplaintById:incrementView");
 ...
 */
export async function updateComplaint(id, requester, input) {
    const existing = await dbGuard(() => prisma.complaint.findUnique({ where: { id }, select: { id: true, userId: true, deletedAt: true } }), "updateComplaint:find");
    ...
    const updated = await dbGuard(() => prisma.complaint.update({
        where: { id },
        data,
        include: { ...COMPLAINT_INCLUDE, user: { select: { fullName: true } } },
 ...
 */
export async function deleteComplaint(id, requester) {
    const existing = await dbGuard(() => prisma.complaint.findUnique({ where: { id }, select: { id: true, userId: true, deletedAt: true } }), "deleteComplaint:find");
    if (!existing || existing.deletedAt) {
        throw new AppError("Complaint not found.", 404, { code: "NOT_FOUND" });
    }
    if (existing.userId !== requester.id && requester.role !== "ADMIN") {
    ...
    await dbGuard(() => prisma.complaint.update({
        where: { id },
