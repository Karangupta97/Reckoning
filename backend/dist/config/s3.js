...
export function assertS3Configured() {
    if (!BUCKET_NAME) {
        throw new AppError("File storage is not configured on this server.", 503, { code: "STORAGE_UNAVAILABLE" });
    }
}
/**
 ...
 */
export async function getSignedDownloadUrl(key, expiresIn = MAX_PRESIGN_SECONDS) {
    assertS3Configured();
