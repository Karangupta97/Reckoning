 ...
 */
export function startAuthorityNotifyWorker() {
    if (!connection) {
        // eslint-disable-next-line no-console
        console.warn("[authorityNotify.worker] REDIS_URL not set — worker not started.");
        return null;
    }
    const worker = new Worker(QUEUE_NAMES.authorityNotify, async (job) => {
        ...
        if (!complaint || !authority || !authority.isActive) {
            // eslint-disable-next-line no-console
            console.warn(`[authorityNotify.worker] Skipping job — missing/inactive target (complaint=${complaintId}, authority=${authorityId}).`);
            return;
        }
        // eslint-disable-next-line no-console
        console.log(`[authorityNotify.worker] Notifying ${authority.name} <${authority.email}> ` +
...
//# sourceMappingURL=authorityNotify.worker.js.map